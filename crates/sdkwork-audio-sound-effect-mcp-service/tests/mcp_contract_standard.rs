...
use sdkwork_audio_sound_effect_generation_service::{
    SoundEffectGenerationCommand, SoundEffectGenerationServicePort, SoundEffectProviderDescriptor,
    SoundEffectProviderResult, SoundEffectProviderSubmission,
};
use sdkwork_audio_sound_effect_mcp_service::{
    streamable_http_service, GenerateSoundEffectInput, SoundEffectGenerationMcpService,
};
use std::sync::Arc;
use tower::ServiceExt;
struct FakeService;
#[async_trait]
impl SoundEffectGenerationServicePort for FakeService {
    async fn generate(
        &self,
        _: SoundEffectGenerationCommand,
    ) -> SoundEffectProviderResult<SoundEffectProviderSubmission> {
        unreachable!()
    }
...
fn service() -> SoundEffectGenerationMcpService {
    SoundEffectGenerationMcpService::new(Arc::new(FakeService))
}
#[test]
fn public_contract_is_provider_neutral() {
    ...
    assert_eq!(
        names,
        ["sound_effect.capabilities", "sound_effect.generate"]
    );
...
fn unified_input_maps_vendor_extensions_without_sdk_dtos() {
    let input: GenerateSoundEffectInput = serde_json::from_value(serde_json::json!({
        "vendor": "Example_Vendor",
        "description": "soft impact",
        "vendorParameters": { "schema": "urn:example:sound-effect:v1", "values": { "seed": 7 } }
    }))
    .unwrap();
    let command: SoundEffectGenerationCommand = input.try_into().unwrap();
    assert_eq!(command.vendor.as_str(), "example-vendor");
    assert_eq!(
        command.vendor_parameters.unwrap().schema,
        "urn:example:sound-effect:v1"
    );
}
